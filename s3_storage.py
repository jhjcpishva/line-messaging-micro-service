...
class S3Storage:
    ...
    def __init__(self, host: str, access_key: str, secret_key: str, logger: logging.Logger, secure: bool = False, public_url=""):
        ...
        self.logger = logger
        self.logger.info(
            f"Initializing S3 client with host: {host}, access_key: {access_key[:4]}..., secret_key: {secret_key[:4]}..., secure: {secure}")
        self.client = Minio(
            host,
    ...
    def put_file(self, bucket_name, file_name, content: BinaryIO, length: int, content_type: str = None, metadata: Dict = None) -> CreatedFileObject:
        result = self.client.put_object(
            bucket_name, file_name, content, length, content_type=content_type, metadata=metadata)
